...
public class IMChatBot {
     ...
     * @throws Exception
     */
    public ApiResult<Message> sendFile(String chatId, long fileSize, String fileName, String fileType, String filePath) throws Exception {
        JSONObject req = new JSONObject();
        req.put("chat_id", chatId);
        req.put("file_size", fileSize);
        req.put("file_name", fileName);
        req.put("type", fileType);
        req.put("file_path", filePath);
        String s = HttpClient.post(getApi("sendFile"), req.toJSONString());
        return getResult(s, ApiResult.ResultTypes.MESSAGE);
    }
     ...
     * @throws Exception
     */
    public ApiResult newChatMermber(String chatId, String userId) throws Exception {
        JSONObject req = new JSONObject();
        req.put("chat_id", chatId);
        req.put("user_id", userId);
        String s = HttpClient.post(getApi("newChatMermber"), req.toJSONString());
        return getResult(s, new TypeReference<ApiResult<String>>() {
        }.getType());
    }
     ...
     * @throws Exception
     */
    public ApiResult kickChatMermber(String chatId, String userId) throws Exception {
        JSONObject req = new JSONObject();
        req.put("chat_id", chatId);
        req.put("user_id", userId);
        String s = HttpClient.post(getApi("kickChatMermber"), req.toJSONString());
        return getResult(s, new TypeReference<ApiResult<String>>() {
        }.getType());
    }

    private ApiResult getResult(String res, ApiResult.ResultTypes resultType) {
        return getResult(res, resultType.getType());
    }

    private ApiResult getResult(String res, Type resultType) {
        try {
            if (StringUtils.isNotEmpty(res)) {
                ApiResult result = JSONObject.parseObject(res, resultType);
                return result;
            }
        } catch (Exception ex) {
