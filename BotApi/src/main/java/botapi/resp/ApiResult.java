...
public class ApiResult<T> {
    ...
    public ApiResult(boolean ok, String description, int errorCode, T result) {
        this.ok = ok;
    ...
    public enum ResultTypes {
        ...
        UPLOAD(new TypeReference<ApiResult<Upload>>() {
        }.getType()),
        LIST_OF_UPDATES(new TypeReference<ApiResult<List<Message>>>() {
        }.getType()),
        CHATS(new TypeReference<ApiResult<List<Chat>>>() {
        }.getType()),
        ME(new TypeReference<ApiResult<User>>() {
        }.getType());
        private Type type;
